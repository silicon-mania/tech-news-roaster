 ...
 * and the bearer-authenticated /enrich service route). When Supabase is not
 * configured the gate stays open, so local fixture development and the test
 * suite keep working without an auth backend.
 */
...
const publicPathPrefixes = [signInPath, "/api/auth", "/enrich"];
