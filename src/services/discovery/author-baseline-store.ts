import "server-only";

import { createClient } from "@supabase/supabase-js";
import { readSupabaseConfig } from "@/services/auth";
import { getOperatorSession } from "@/services/auth/operator-session";
import { type OperatorSessionReader, resolveOwnerId } from "@/services/saved-runs/run-repository";
import type { AuthorBaselineRepository } from "./author-baseline-repository";
import { createInMemoryAuthorBaselineRepository } from "./in-memory-author-baseline-repository";
import { createSupabaseAuthorBaselineRepository } from "./supabase-author-baseline-repository";

type SupabaseEnvironment = Readonly<Record<string, string | undefined>>;

/**
 * Builds an owner-scoped {@link AuthorBaselineRepository}. With Supabase configured
 * it is the Postgres store reached with the service-role key; otherwise it is the
 * in-memory fallback so local fixture development works without a backend. Mirrors
 * createRunRepository in saved-runs.
 */
export function createAuthorBaselineRepository(
  ownerId: string,
  env: SupabaseEnvironment = process.env,
): AuthorBaselineRepository {
  const config = readSupabaseConfig(env);

  if (!config) {
    return createInMemoryAuthorBaselineRepository(ownerId);
  }

  const client = createClient(config.url, config.serviceRoleKey, {
    auth: { persistSession: false },
  });

  return createSupabaseAuthorBaselineRepository(ownerId, client);
}

export type AuthorBaselineRepositoryResolution =
  | { repository: AuthorBaselineRepository }
  | { unauthorized: true };

/**
 * Resolves the Operator Account behind a request and hands back its baseline
 * repository, or rejects when Supabase is configured but no operator is signed in
 * — the same owner resolution the Saved Run repository uses, so a run and its
 * authors' baselines land under one owner.
 */
export async function resolveAuthorBaselineRepository(
  env: SupabaseEnvironment = process.env,
  getSession: OperatorSessionReader = getOperatorSession,
): Promise<AuthorBaselineRepositoryResolution> {
  const owner = await resolveOwnerId(env, getSession);

  if ("unauthorized" in owner) {
    return { unauthorized: true };
  }

  return { repository: createAuthorBaselineRepository(owner.ownerId, env) };
}
